...
export function PinModal({ onSuccess, onCancel, canCancel = true, mode = 'login' }: PinModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isSetup) {
      if (!/^\d{4,10}$/.test(pin)) { setError(t.pinInvalidFormat); return; }
      if (pin !== confirmPin) { setError(t.pinMismatch); return; }
    }
    setLoading(true);
    setError('');
    try {
      const { token } = isSetup ? await api.setupPin(pin) : await api.login(pin, rememberMe);
      onSuccess(token);
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className={`w-full max-w-sm mx-4 rounded-2xl shadow-2xl p-8 ${dm ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading || !pin.trim() || (isSetup && !confirmPin.trim()) || isLocked}
          ...
          >
            {loading
              ? (isSetup ? t.pinSettingUp : t.signingIn)
              : isLocked ? t.waitSeconds(retryAfter) : (isSetup ? t.pinSetupSubmit : t.signIn)}
          </button>
          {canCancel && (
            <button
              type="button"
